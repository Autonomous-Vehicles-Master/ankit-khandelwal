...
def plot_dataset(vehicle_id):
    ...
    df = pd.DataFrame(ds,columns=['Local_X','Indexer'])
    df.plot(x ='Indexer', y='Local_X', kind = 'line')     
    df = pd.DataFrame(ds,columns=['x_hat','Indexer'])
    df.plot(x ='Indexer', y='x_hat', kind = 'line') 
    
    df = pd.DataFrame(ds,columns=['x_Vel','Indexer'])
    df.plot(x ='Indexer', y='x_Vel', kind = 'line')      
    df = pd.DataFrame(ds,columns=['x_Vel_hat','Indexer'])
    df.plot(x ='Indexer', y='x_Vel_hat', kind = 'line') 
    
    df = pd.DataFrame(ds,columns=['y_Vel','Indexer'])
    df.plot(x ='Indexer', y='y_Vel', kind = 'line')      
